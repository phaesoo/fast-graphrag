...
from fast_graphrag._utils import (
...
class TestExtractSortedScores(unittest.TestCase):
    ...
    def test_duplicate_elements(self):
        ...
        self.assertTrue(
            np.array_equal(indices, expected_indices_1)
            or np.array_equal(indices, expected_indices_2),
...
class TestCsrFromListOfLists(unittest.TestCase):
    def test_repeated_elements(self):
        ...
        np.testing.assert_array_equal(
            result_matrix.toarray(), expected_matrix.toarray()
        )

    def test_non_zero_elements(self):
        data = [[0, 1, 2], [2, 3], [0, 3]]
        expected_matrix = csr_matrix(
            [[1, 1, 1, 0, 0], [0, 0, 1, 1, 0], [1, 0, 0, 1, 0]], shape=(3, 5)
        )
