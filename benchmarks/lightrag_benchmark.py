    ...
    if args.benchmark:
        ...
        async def _query_task(query: Query, mode: str) -> Dict[str, Any]:
            answer = await grag.aquery(
                query.question,
                QueryParam(
                    mode=mode, only_need_context=True, max_token_for_text_unit=9000
                ),
            )
            chunks = [
        ...
        with open(
            f"./results/lightrag/{args.dataset}_{args.n}_{args.mode}.json", "w"
        ) as f:
            json.dump(answers, f, indent=4)

    if args.benchmark or args.score:
        with open(
            f"./results/lightrag/{args.dataset}_{args.n}_{args.mode}.json", "r"
        ) as f:
            answers = json.load(f)
